    ...
    return (
      ...
      >
        ...
        <Box sx={{ width: "100%", margin: "8px 32px" }}>
          ...
          <progress
            max={100}
            value={meta.status === "done" || meta.status === "headers_received" ? 100 : meta.percent}
            className={errorStatus.includes(meta.status) ? "error" : ""}
          />
        </Box>
        ...
        {meta.status === "uploading" && (
          <Box onClick={cancel}>
            <RestartAltOutlinedIcon sx={{ color: "#939393" }} fontSize="small" />
          </Box>
        )}
        {meta.status !== "preparing" && meta.status !== "getting_upload_params" && meta.status !== "uploading" && (
          <Box onClick={remove}>
